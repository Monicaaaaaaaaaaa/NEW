      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col fixed h-full">
        ...
        <nav className="flex-1 px-4 py-2 space-y-0.5 overflow-y-auto">
          <button onClick={() => router.push("/dashboard")}
            ...
            <Home size={17} strokeWidth={1.6} /><span>Dashboard</span>
          </button>

          <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase px-3 pt-5 pb-2">Your Business</p>

          <button onClick={() => router.push("/roles")}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#4b5d79] hover:bg-blue-50 hover:text-blue-600 text-sm transition-colors">
            <ShieldCheck size={17} strokeWidth={1.6} /><span>Roles</span>
          </button>

          <button onClick={() => router.push("/vat-automation")}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#4b5d79] hover:bg-blue-50 hover:text-blue-600 text-sm transition-colors">
            <Calculator size={17} strokeWidth={1.6} /><span>VAT Automation</span>
          </button>
          ...
          <button onClick={() => router.push("/invoicing")}
            ...
            <span className="flex items-center gap-3"><FileText size={17} strokeWidth={1.6} />E-Invoicing</span>
            <ChevronRight size={14} strokeWidth={1.6} />
          </button>

          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-white text-sm font-medium"
            style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #1e40af 100%)" }}>
            <Users size={17} strokeWidth={1.6} /><span>User Management</span>
          </button>

          <button onClick={() => router.push("/report")}
            className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-[#4b5d79] hover:bg-blue-50 hover:text-blue-600 text-sm transition-colors">
            <span className="flex items-center gap-3"><BarChart2 size={17} strokeWidth={1.6} />Report</span>
            <ChevronRight size={14} strokeWidth={1.6} />
          </button>

          <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase px-3 pt-5 pb-2">Others</p>

          <button onClick={() => router.push("/settings")}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#4b5d79] hover:bg-blue-50 hover:text-blue-600 text-sm transition-colors">
            <Settings size={17} strokeWidth={1.6} /><span>Settings</span>
          </button>
        </nav>
        ...
        <div className="flex items-center gap-3 px-3 pb-4">
          <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-white text-xs font-semibold"
            style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #1e40af 100%)" }}>AS</div>
          <div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <button className="flex items-center gap-2 text-sm font-bold text-gray-800 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">
              <SlidersHorizontal size={14} />Column
            </button>
          </div>
          ...
          {error && (
            <div className="mx-6 mt-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded-lg text-sm flex items-center justify-between">
              <span>{error}</span>
              <button onClick={() => fetchUsers(currentPage, rowsPerPage, search, filter)} className="text-red-700 underline text-xs">Retry</button>
            </div>
          )}
              ...
              {loading ? (
                Array.from({ length: rowsPerPage }).map((_, i) => (
                  <tr key={i} className="border-b border-gray-100">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <td key={j} className="px-6 py-4"><div className="h-4 bg-gray-100 animate-pulse rounded w-24" /></td>
                    ))}
                  </tr>
                ))
              ) : users.length === 0 ? (
                ...
                users.map((user, i) => (
                  <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50/60 transition-colors">
                    <td className="px-6 py-4 text-gray-400 font-medium">{(currentPage - 1) * rowsPerPage + i + 1}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 text-sm font-medium ${user.status === "Active" ? "text-green-600" : "text-red-500"}`}>
                        <span className={`w-2 h-2 rounded-full ${user.status === "Active" ? "bg-green-500" : "bg-red-500"}`} />
                        {user.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 relative">
                      <button onClick={() => setOpenActionId(openActionId === user.id ? null : user.id)}
                      ...
                      {openActionId === user.id && (
                        <div className="absolute right-10 top-8 bg-white border border-gray-200 rounded-xl shadow-lg z-20 w-44 py-1.5 overflow-hidden">
                          <button className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                            <Pencil size={13} className="text-blue-500" />Edit User
                          </button>
          ...
          <div className="flex items-center justify-between px-6 py-4 text-sm text-gray-500">
            ...
            <div className="flex items-center gap-1">
              ...
              <button className="px-3 py-1 text-white rounded-lg font-medium min-w-[32px]"
                style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #1e40af 100%)" }}>
                {currentPage}
              </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4">
            ...
            <div className="px-7 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="tel" placeholder="Enter phone number" value={form.mobileNumber}
